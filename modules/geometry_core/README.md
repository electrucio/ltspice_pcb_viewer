...
Measured on the poweramp board, B.Cu, 1 mm target, with `pcb_mesh`'s default outline
simplification (0.01 mm): **54k triangles, 0 slivers, min ∠ 25°, ~0.3 s end-to-end** —
against the TS cdt2d path's 38k triangles with **31%** slivers in 2.1 s. Without
simplification Ruppert resolves every dense fill vertex as a local feature (212k
triangles) — an optional `min_area` refinement floor exists but measurement showed
simplification is the better knob (the floor only reintroduces slivers).
